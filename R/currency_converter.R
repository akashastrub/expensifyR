...
#'
#' @param df Dataframe with multiple currency columns, one of which (currency_in)
#' is to be converted to the other (curency_out).
...
convert_amount <- function(df, currency_in, currency_out) {
  ...
  df_ex <- priceR::historical_exchange_rates(toupper(currency_in),
  ...
  currency_in_var_name <-
    stringr::str_c("amount", tolower(currency_in), sep = "_")
  ...
  df <- df %>%
    ...
    dplyr::mutate(amount_original = round(amount_original * exchange, 2)) %>%
    dplyr::select(-exchange)

  # Convert currency-specific column name in df to generic name for output
  currency_out_var_name <-
    stringr::str_c("amount", tolower(currency_out), sep = "_")
